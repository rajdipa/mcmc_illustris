...
from parameters import ProfileParams
...
__all__ = ['Sersic', 'lnlike_prof', 'norm_prof', 'config_params', 'ln_probability',
...
def norm_prof(rad, rho, min_r=6.0, max_r=120.0):
...
def config_params(rad, rho, err, min_r=6.0, max_r=120.0,
    ...
    if param_config is None:
        ...
        #   I0: Central intensity, flat prior between the min and max range decided
        #       on normalized profile.
        #   Re: Effective radius, flat prior between the min and max fitting range;
        #       initial guess is 10.0
        # TODO: This should be replaced with more appropriate priors
        param_config = {
            'n': {
                'name': 'n', 'label':r'$n_{\rm Ser}$', 'ini': 3.0,
                'min': 1.0, 'max': 10.0, 'type': 'flat', 'sig': 1.0
            },
            'I0': {
            ...
            'Re': {
                'name': 'Re', 'label':r'$R_{\rm e}$', 'ini': 10.0,
                'min': min_r, 'max': 250, 'type': 'flat', 'sig': 20.
            }
        }
...
def prof_curvefit(func, rad, rho, err, params, min_r=6.0, max_r=120.0):
    ...
    best_curvefit, cov_curvefit = curve_fit(
        func, rad[flag], rho[flag], p0=params.get_avg(),
...
def update_params(pbest, pcov, nsig=5.0):
    ...
    param_config = {
        'n': {
            ...
            'min': pbest[0] - nsig * perr[0], 'max': pbest[0] + 1* nsig * perr[0],
        ...
        'Re': {
            ...
            'min': pbest[2] - nsig * perr[2], 'max': pbest[2] + 2.5* nsig * perr[2],
...
def emcee_fit_one_sersic(rad, rho, err, min_r=6.0, max_r=120.0, pool=None,
    ...
    assert params.names == ['n', 'I0', 'Re'], "Wrong parameter order!"
    ...
    # Put the normalized profiles in the results
    results['prof_norm'] = rho_norm
...
def visual_emcee(results, burnin=None, fontsize=20, alpha=0.3):
    ...
    mod_corner = plot_mcmc_corner(
        ...
        fontsize=fontsize, labelsize=22,
...
def display_model_1d(rad, rho, err, min_r=6.0, max_r=100.0, log_r=True,
